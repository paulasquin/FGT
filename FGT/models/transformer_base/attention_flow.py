...
class SWMHSA_depthGlobalWindowConcatLN_qkFlow_reweightFlow(nn.Module):
    def __init__(
        self, token_size, window_size, kernel_size, d_model, flow_dModel, head, p=0.1
    ):
        ...
        self.group_h, self.group_w = (
            self.new_h // self.window_size,
            self.new_w // self.window_size,
        )
        self.global_extract_v = nn.Conv2d(
            d_model,
            d_model,
        ...
        self.global_extract_k = nn.Conv2d(
            in_channels,
            in_channels,
            kernel_size=kernel_size,
            stride=kernel_size,
        ...
        self.reweightFlow = nn.Sequential(
            nn.Linear(in_channels, flow_dModel), nn.Sigmoid()
        )
    ...
    def inference(self, x, f, h, w):
        ...
        # generate q
        q = qk.reshape(
            bt, group_h, self.window_size, group_w, self.window_size, qk_c
        ).transpose(2, 3)
        ...
        k_global = (
            k_global.permute(0, 2, 3, 1)
            .reshape(bt, -1, qk_c)
            .unsqueeze(1)
            .repeat(1, group_h * group_w, 1, 1)
        )
        ...
        global_tokens = (
            global_tokens.permute(0, 2, 3, 1)
            .reshape(bt, -1, c)
            .unsqueeze(1)
            .repeat(1, group_h * group_w, 1, 1)
        )  # [bt, gh * gw, h'*w', c]
        x = x.reshape(
            bt, group_h, self.window_size, group_w, self.window_size, c
        ).transpose(
            2, 3
        )  # [bt, gh, gw, ws, ws, c]
        x = x.reshape(
            bt, group_h * group_w, self.window_size * self.window_size, c
        )  # [bt, gh * gw, ws^2, c]
        v = torch.cat((x, global_tokens), dim=2)
        v = self.v_norm(v)
        query = self.query_embedding(
            q
        )  # [bt, self.group_h, self.group_w, self.window_size, self.window_size, c]
        ...
        key = key.reshape(bt, group_h * group_w, -1, self.head, c // self.head).permute(
            0, 1, 3, 2, 4
        )
        value = value.reshape(
            bt, group_h * group_w, -1, self.head, c // self.head
        ).permute(0, 1, 3, 2, 4)
    ...
    def forward(self, x, f, t, h=0, w=0):
        ...
        if self.pad_r > 0 or self.pad_b > 0:
            ...
            f = F.pad(
                f, (0, 0, self.pad_l, self.pad_r, self.pad_t, self.pad_b)
            )  # [bt, cf, h, w]
        y = x.permute(0, 3, 1, 2)
        xf = torch.cat((x, f), dim=-1)
        ...
        # generate q
        q = qk.reshape(
            bt, self.group_h, self.window_size, self.group_w, self.window_size, qk_c
        ).transpose(2, 3)
        q = q.reshape(
            bt, self.group_h * self.group_w, self.window_size * self.window_size, qk_c
        )
        # generate k
        ky = qk.permute(0, 3, 1, 2)  # [b, c, h, w]
        ...
        k_global = (
            ...
            .reshape(bt, -1, qk_c)
            .unsqueeze(1)
            .repeat(1, self.group_h * self.group_w, 1, 1)
        )
        k = torch.cat((q, k_global), dim=2)
        # norm q and k
        q = self.q_norm(q)
        ...
        x = x.reshape(
            bt, self.group_h, self.window_size, self.group_w, self.window_size, c
        ).transpose(
            2, 3
        )  # [bt, gh, gw, ws, ws, c]
        x = x.reshape(
        ...
        key = key.reshape(
            bt, self.group_h * self.group_w, -1, self.head, c // self.head
        ).permute(0, 1, 3, 2, 4)
        value = value.reshape(
            bt, self.group_h * self.group_w, -1, self.head, c // self.head
        ).permute(0, 1, 3, 2, 4)
