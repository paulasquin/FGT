...
class Attention(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        norm = math.sqrt(query.size(-1))
        del key
        del query
        scores = F.softmax(scores / norm, dim=-1)
        scores = self.dropout(scores)
        p_val = torch.matmul(scores, value)
        return p_val, scores
...
class TMHSA(nn.Module):
    def __init__(self, token_size, group_size, d_model, head, p=0.1):
        ...
        self.wh, self.ww = math.ceil(self.h / self.group_size), math.ceil(
            self.w / self.group_size
        )
        ...
        self.window_h, self.window_w = (
            self.new_h // self.group_size,
            self.new_w // self.group_size,
        )
    ...
    def inference(self, x, t, h, w):
        ...
        if pad_r > 0 or pad_b > 0:
            x = F.pad(
                x, (0, 0, 0, pad_r, 0, pad_b)
            )  # channel, channel, left, right, top, bottom -> [bt, new_h, new_w, c]
        query = self.query_embedding(x)
        key = self.key_embedding(x)
        value = self.value_embedding(x)
        query = query.view(
            b, t, self.group_size, window_h, self.group_size, window_w, self.head, c_h
        )
        query = query.permute(0, 2, 4, 6, 1, 3, 5, 7).reshape(
            b, self.group_size * self.group_size, self.head, -1, c_h
        )
        key = key.view(
            b, t, self.group_size, window_h, self.group_size, window_w, self.head, c_h
        )
        key = key.permute(0, 2, 4, 6, 1, 3, 5, 7).reshape(
            b, self.group_size * self.group_size, self.head, -1, c_h
        )
        value = value.view(
            b, t, self.group_size, window_h, self.group_size, window_w, self.head, c_h
        )
        value = value.permute(0, 2, 4, 6, 1, 3, 5, 7).reshape(
            b, self.group_size * self.group_size, self.head, -1, c_h
        )
        att, _ = self.attention(query, key, value)
        att = att.view(
            b, self.group_size, self.group_size, self.head, t, window_h, window_w, c_h
        )
    ...
    def forward(self, x, t, h=0, w=0):
        ...
        if h == 0 and w == 0:
            assert (
                n == self.h * self.w
            ), "Wrong input shape: {} with token: h->{}, w->{}".format(
                x.shape, self.h, self.w
            )
        else:
            assert n == h * w, "Wrong input shape: {} with token: h->{}, w->{}".format(
                x.shape, h, w
            )
        ...
        if self.pad_r > 0 or self.pad_b > 0:
            x = F.pad(
                x, (0, 0, 0, self.pad_r, 0, self.pad_b)
            )  # channel, channel, left, right, top, bottom -> [bt, new_h, new_w, c]
        query = self.query_embedding(x)
        key = self.key_embedding(x)
        value = self.value_embedding(x)
        ...
        att = (
            att.permute(0, 4, 1, 5, 2, 6, 3, 7)
            .contiguous()
            .view(bt, self.new_h, self.new_w, c)
        )
