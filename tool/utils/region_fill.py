...
def regionfill(I, mask, factor=1.0):  # I -> flow, mask -> flow mask
    ...
    maskPerimeter = findBoundaryPixels(
        resize_mask
    )  # boundary pixels in the resized mask
...
def formRightSide(I, maskPerimeter):
    ...
    assert perimeterValues.shape == I.shape, "P shape: {}, I shape: {}".format(
        perimeterValues.shape, I.shape
    )
    ...
    rightSide[height - 1, width - 1] = (
        perimeterValues[height - 2, width - 1] + perimeterValues[height - 1, width - 2]
    )
    return rightSide
