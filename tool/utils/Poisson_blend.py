...
def constructEquation(
    ...
    if n == 0:
        b_tmp = (
            -imgSrc_gx[pi[valid, 0], pj[valid, 0], :]
            + imgTrg[qi[valid, n], qj[valid, n], :]
        )
    elif n == 2:
        b_tmp = (
            imgSrc_gx[pi[valid, 0], pj[valid, 0] - 1, :]
            + imgTrg[qi[valid, n], qj[valid, n], :]
        )
    elif n == 1:
        b_tmp = (
            -imgSrc_gy[pi[valid, 0], pj[valid, 0], :]
            + imgTrg[qi[valid, n], qj[valid, n], :]
        )
    elif n == 3:
        b_tmp = (
            imgSrc_gy[pi[valid, 0] - 1, pj[valid, 0], :]
            + imgTrg[qi[valid, n], qj[valid, n], :]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--flow", type=str, default="../../test_blending/flow/00000.flo"
    )
    parser.add_argument(
        "--mask", type=str, default="../../test_blending/mask/00000.png"
    )
    ...
    flow_resized[:, :, 0] = (
        cv2.resize(flow[:, :, 0], (width, height), cv2.INTER_LINEAR) * width / w
    )
    flow_resized[:, :, 1] = (
        cv2.resize(flow[:, :, 1], (width, height), cv2.INTER_LINEAR) * height / h
    )
    flow = flow_resized
    ...
    gradient_x = np.concatenate(
        (np.diff(flow, axis=1), np.zeros((height, 1, 2), dtype=np.float32)), axis=1
    )
    gradient_y = np.concatenate(
        (np.diff(flow, axis=0), np.zeros((1, width, 2), dtype=np.float32)), axis=0
    )
    ...
    compFlow = Poisson_blend(
        flow, imgSrc_gx, imgSrc_gy, mask, masked_edge
    )  # todo: edge or masked_edge ?
