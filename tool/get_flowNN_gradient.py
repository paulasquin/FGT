...
def get_flowNN_gradient(
    ...
    NN_idx = 0  # BN:0
    for indFrame in (bar := tqdm(frameIndSetF)):
        ...
        BFdiff, BF_uv = consistCheck(
            videoFlowF[:, :, :, indFrame - 1], videoFlowB[:, :, :, indFrame - 1]
        )"""
        _, BF_uv = consistCheck(
            videoFlowF[:, :, :, indFrame - 1], videoFlowB[:, :, :, indFrame - 1]
        )
        ...
        KnownInd = (
            mask[flow_neighbor_int[:, 0], flow_neighbor_int[:, 1], indFrame - 1] == 0
        )
        ...
        flowNN[
            numPixInd[
                holepixPos[KnownIsConsist, 0], holepixPos[KnownIsConsist, 1], indFrame
            ].astype(np.int32),
            :,
            NN_idx,
        ...
        consistency_uv[
            ...
            BF_uv[holepixPos[KnownIsConsist, 0], holepixPos[KnownIsConsist, 1], 0]
        )
        consistency_uv[
            holepixPos[KnownIsConsist, 0],
            holepixPos[KnownIsConsist, 1],
            ...
            BF_uv[holepixPos[KnownIsConsist, 0], holepixPos[KnownIsConsist, 1], 1]
        )
        ...
        HaveNNInd = (
            HaveFlowNN[
                flow_neighbor_int[:, 0], flow_neighbor_int[:, 1], indFrame - 1, NN_idx
            ]
            == 1
        )
        ...
        flowNN_tmp = copy.deepcopy(
            flowNN[
                numPixInd[
                    flow_neighbor_int[:, 0], flow_neighbor_int[:, 1], indFrame - 1
                ].astype(np.int32),
                :,
                NN_idx,
            ]
        ...
        flowNN[
            numPixInd[holepixPos[Valid, 0], holepixPos[Valid, 1], indFrame].astype(
                np.int32
            ),
            :,
            NN_idx,
        ...
        consistency_uv[
            holepixPos[Valid, 0], holepixPos[Valid, 1], NN_idx, 0, indFrame
        ] = np.maximum(
        ...
        consistency_uv[
            holepixPos[Valid, 0], holepixPos[Valid, 1], NN_idx, 1, indFrame
        ] = np.maximum(
        ...
        consistencyMap[:, :, NN_idx, indFrame] = (
            ...
            + consistency_uv[:, :, NN_idx, 1, indFrame] ** 2
        ) ** 0.5
    ...
    NN_idx = 1  # FN:1
    for indFrame in (bar := tqdm(frameIndSetB)):
        # Bool indicator of missing pixels at frame t
        holepixPosInd = sub[:, 2] == indFrame
        ...
        """FBdiff, FB_uv = consistCheck(
            videoFlowB[:, :, :, indFrame], videoFlowF[:, :, :, indFrame]
        )"""
        _, FB_uv = consistCheck(
            videoFlowB[:, :, :, indFrame], videoFlowF[:, :, :, indFrame]
        )
        ...
        consistency_uv[
            ...
            FB_uv[holepixPos[KnownIsConsist, 0], holepixPos[KnownIsConsist, 1], 0]
        )
        consistency_uv[
            holepixPos[KnownIsConsist, 0],
            holepixPos[KnownIsConsist, 1],
            ...
            FB_uv[holepixPos[KnownIsConsist, 0], holepixPos[KnownIsConsist, 1], 1]
        )
    ...
    for indFrame in (bar := tqdm(range(nFrame))):
        ...
        bar.set_description(
            "{0:8d} pixels are from source Frame {1:3d}".format(
    ...
    for indFrame in (bar := tqdm(range(nFrame - 1, -1, -1))):
    ...
    for indFrame in range(nFrame):
        ...
        if args.Nonlocal:
            gradient_x_Candidate[:, :, :, 2] = gradient_x_KeySourceFrameFlowNN[
                :, :, :, 0
            ]
            gradient_y_Candidate[:, :, :, 2] = gradient_y_KeySourceFrameFlowNN[
                :, :, :, 0
            ]
            gradient_x_Candidate[:, :, :, 3] = gradient_x_KeySourceFrameFlowNN[
                :, :, :, 1
            ]
            gradient_y_Candidate[:, :, :, 3] = gradient_y_KeySourceFrameFlowNN[
                :, :, :, 1
            ]
            gradient_x_Candidate[:, :, :, 4] = gradient_x_KeySourceFrameFlowNN[
                :, :, :, 2
            ]
            gradient_y_Candidate[:, :, :, 4] = gradient_y_KeySourceFrameFlowNN[
                :, :, :, 2
            ]
        ...
        weights = (consistencyMap[HaveNN_sum, :, indFrame] * HaveNN[HaveNN_sum, :]) / (
            (consistencyMap[HaveNN_sum, :, indFrame] * HaveNN[HaveNN_sum, :]).sum(
        ...
        weights[fix, :] = HaveNN[HaveNN_sum, :][fix, :] / HaveNN[HaveNN_sum, :][
            fix, :
        ...
        mask_tofill[
            np.logical_and(np.invert(HaveNN_sum), mask[:, :, indFrame]), indFrame
        ] = True
